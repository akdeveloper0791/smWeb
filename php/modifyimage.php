<?php
	...
	if($media_name!=null)
	{
		...
		if(isset($_FILES["AudiofileName"]['name']))
		{
			...
			$newname = "DNDM-".$media_name.".".$audio_ext;

			$target = $directory.$newname;
				
			move_uploaded_file($_FILES['AudiofileName']['tmp_name'], $target);

			//rename($_FILES['AudiofileName']['tmp_name'], $target);
				
			copy($target, $directory.$newname);
		}
